...
class Calculator3():
  ...
  def calculate(self, request: FlaskRequest) -> Dict: # type: ignore
    ...
    formated_response = self.__format_repsonse(variance)
  ...
  def __verify_results(self, variance: float, multiplication: float) -> None:
    if variance < multiplication:
      raise HttpBadRequestError("Falha no processo: Variancia menor que a multiplicação")
    
  def __format_repsonse(self, variance: float) -> Dict:
    return{
      "data": {
        "Calculator" : 3,
